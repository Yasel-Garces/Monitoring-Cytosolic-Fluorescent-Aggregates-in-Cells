...
# -------------------------------------------------------------------
# Some variables will be converted to factors.
...
p1<-p1+geom_point()+geom_line()+xlab('Time')+ylab('Number of Lysosomes')+
  ...
  scale_x_continuous(limits = c(0, 605),breaks = seq(0, 600, by = 50))
ggsave("Number_of_Lysosomes.pdf",p1,scale = 1,width = 94,height=70,
       units = "mm")
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
# Resume all the information by cells, that is, for each cell we compute the number of lysosomes, 
# the time, mean of the area, perimeter mean, intensity mean, integral intensity mean and mean of 
# intensity over the area.
